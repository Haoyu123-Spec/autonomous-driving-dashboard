...
class MultiAGVEnv:
    ...
    def reset(self):
        ...
        self.priority = np.zeros(cfg.n_agvs, dtype=bool)  # True=高优先级
        self.step_count = 0
    ...
    def _get_obs_dim(self):
        cfg = self.cfg
        return (4 + (cfg.max_agents - 1) * 2 + cfg.n_dynamic_obs * 2
                + int(cfg.enable_priority))
        ...
        for i in range(cfg.n_agvs):
            ...
            for j in range(cfg.n_agvs):
                if j != i:
                    ...
                    others.append((np.linalg.norm(rel), rel, self.priority[j]))
            others.sort(key=lambda x: x[0])
            for k in range(min(len(others), cfg.max_agents - 1)):
                _, rel, is_prio = others[k]
                s[idx:idx+2] = rel
                idx += 2
